...
The goal is a cross-platform homie. The app is already portable — roughly 4% of
`homie-app` is macOS-specific and it is cfg-gated. What binds homie to macOS is
the *engine*: the Swift `homied` stack in `Sources/`, which owns PTYs,
sessions, detection and the control socket.

This is the record of replacing it with `crates/homie-engine`.
...
3. **Rules stay data.** Detection manifests are read from
   `Sources/HomieCore/Resources/manifests/`, not copied. One source of truth,
   no drift.
...
| Holder (session survival) | **done** | Server, manager, launcher, client — same sockets, NDJSON protocol, pid files, and OSC 777 exit marker as `HomieHolderKit`. Interop-proven live: the Rust client/launcher drives the real Swift `homied-holder` binary (spec encode → Swift decode, stat/write/resize/signal/kill-tree, Swift-written log read back, Swift exit marker parsed) |
...
| Rust daemon binary | **done** | `homied-rs`: same socket, lock singleton, state file, boot-log stamp, holder adoption, manifest bundle + overrides loading. Ships in `Resources/bin` beside the Swift daemon; opt in per machine with `HOMIED_PATH=…/Resources/bin/homied-rs`. Verified live: the Swift `homie` CLI spawned, typed into, read, waited on, and archived a session served entirely by the Rust engine |
...
Since ported into `homied-rs` (2026-08-07 second pass): artifacts scanning
...
Remaining gaps in `homied-rs` (all answer clean errors):
