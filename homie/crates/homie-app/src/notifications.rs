...
pub fn prefs_sync_transition(
    host_name: &str,
    result: Result<&homie_proto::HostSyncPrefsResult, &str>,
) -> StatusTransition {
    match result {
        Ok(report) => {
...
pub fn reach_failure_transition() -> StatusTransition {
    StatusTransition {
        ...
        notification: Some(NotificationRequest {
            ...
            body: "homie couldn't deliver your answer. Open the session to respond.".to_owned(),
            thread_identifier: None,
            action_data: None,
            use_system_sound: true,
        }),
...
fn blocker_identity(detail: &homie_proto::NeedsInputDetail) -> String {
...
mod tests {
    ...
    fn prefs_sync_and_migration_banners_summarize_outcomes() {
        let report = homie_proto::HostSyncPrefsResult {
            tools: vec![
                homie_proto::PrefsSyncToolReport {
                    tool: "claude".into(),
                    ok: true,
