...
fn additive_unknown_variants_fall_back_and_unknown_fields_are_ignored() {
    ...
    let worktree: homie_proto::WorktreeInfo = serde_json::from_value(json!({
        "path": "/tmp/wt",
        "branch": "main",
        "isBare": false,
        "isDetached": false,
...
fn migration_and_host_methods_use_the_swift_wire_names() {
    ...
    let to_local = homie_proto::SessionMigrateParams {
        session_id: homie_proto::SessionId::new("s_1"),
        target_host: None,
    };
    assert_eq!(
    ...
    let to_forge: homie_proto::SessionMigrateParams =
    ...
    let report: homie_proto::HostSyncPrefsResult = serde_json::from_value(json!({
        "tools": [
    ...
    let locate = homie_proto::HostLocateRepoParams {
        host: Some("forge".into()),
        origin_url: None,
        session_id: Some(homie_proto::SessionId::new("s_1")),
    };
    assert_eq!(
        serde_json::to_value(&locate).unwrap(),
        json!({"host": "forge", "sessionID": "s_1"})
    );
    let found: homie_proto::HostLocateRepoResult = serde_json::from_value(
    ...
    let miss: homie_proto::HostLocateRepoResult = serde_json::from_value(json!({})).unwrap();
    assert_eq!(miss, homie_proto::HostLocateRepoResult::default());

    // Repo-preserving spawn field: sameRepoAs spelling, absent ⇒ None.
    let legacy: homie_proto::SessionSpawnParams =
        serde_json::from_value(json!({"kind": {"shell": {}}, "cwd": "/tmp"})).unwrap();
    assert_eq!(legacy.same_repo_as, None);
    let preserving = homie_proto::SessionSpawnParams {
        same_repo_as: Some(homie_proto::SessionId::new("s_ref")),
        ..legacy
    };
    assert_eq!(
