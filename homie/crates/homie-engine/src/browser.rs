    ...
    fn ensure_running(&self, inner: &mut PoolInner) -> Result<(), String> {
        ...
        if let Some(stdout) = stdout {
            ...
            let _ = std::thread::Builder::new()
                .name("homie-browser-sidecar".into())
                .spawn(move || {
...
/// from the executable (dev checkouts run from target dirs under homie/).
fn locate_sidecar() -> Option<PathBuf> {
    if let Ok(configured) = std::env::var("HOMIE_SIDECAR") {
...
mod tests {
    ...
    fn shutdown_wakes_pending_requests_and_is_idempotent() {
        let pool = BrowserPool::new(Path::new("/tmp/homie-browser-test"));
        let (sender, receiver) = mpsc::channel();
        pool.shared
            .inner
            .lock()
