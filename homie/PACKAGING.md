...
`scripts/package.sh` builds `homie` for Apple silicon and Intel, combines the two slices with `lipo`, asks cargo-packager to assemble `dist/homie.app`, and signs the result. The bundle identifier is `com.homie.homie`, the deployment target is macOS 15.0, and the app does not use App Sandbox.
...
```sh
export HOMIE_SIGN_IDENTITY='Developer ID Application: Example Team (TEAMID)'
export HOMIE_CREATE_DMG=1
export APPLE_NOTARIZATION_KEYCHAIN_PROFILE=homie-notary
scripts/package.sh
```
...
Notarizing also produces `dist/homie-<version>-universal.zip`, the artifact homie's updater downloads. `HOMIE_CREATE_ZIP=1` builds it without notarization too. The version in both artifact names comes from `crates/homie-app/Cargo.toml` unless `HOMIE_VERSION` overrides it, because the updater compares against `CARGO_PKG_VERSION`.
