...
fn releases_direct_child() {
    ...
    assert_eq!(released["sessionId"], child_id);
}

#[test]
fn rejects_releasing_calling_session() {
    let temp = TempDir::new().expect("tempdir");
    let _runtime = support::RuntimeGuard::new(temp.path());
    let parent_id = create_session(temp.path(), "Parent");
    ...
    assert!(
        response["error"]["message"]
            .as_str()
            .unwrap_or_default()
            .contains("cannot terminate the calling session")
    );
}
