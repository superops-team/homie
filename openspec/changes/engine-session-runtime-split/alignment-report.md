...
| Thin spawn/resume/migrate handlers | T8 | Done (spawn/resume sunk; migrate orchestration stays in handler) |
...
其 resume 调用已下沉。spawn 组装已全部下沉到 `session/launch.rs`
（`spawn_spec` / `remote_spawn_spec`）。wire 协议与行为不变。
