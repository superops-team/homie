# OpenSpec Tasks — app-sidebar-sections-split
...
- [x] `chrome.rs`（3）、`project.rs`（1）、`session.rs`（2）、`archive.rs`（2）、`footer.rs`（2）。
- 验收：方法体逐字迁移，`pub(crate)` 保留。关联 C1/C5。

## T3 删除旧文件 + 编译验证

- [x] 删除 `sidebar/sections.rs`。
...
- [x] release readiness 证据写入 `docs/verification/app-sidebar-sections-split/`。
- 验收：通过。关联 C6。
