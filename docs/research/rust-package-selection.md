...
| HTTP server | `axum` | daemon-embedded LLM proxy HTTP listener (owner crate `homie-gateway` library, embedded in `homie-engine`) | `json` feature; loopback-only bind |
| HTTP client | `reqwest` | Gateway upstream forwarding (owner crate `homie-gateway` library) | `json` + `stream` features; SSE pass-through |
